...
def check_input(input_str,input_type): # To check user input and validate
    """
   check user input validity
   input_str: input from the user
   input_type:type of input from user: 1 = city, 2 = month, 3 = day
   """
    while True:  # To keep ask user to put input till correct answer
        ...
        try:
            if input_read.lower() in ['chicago','new york city','washington'] and input_type == 1:
                ...
                if input_type == 1:
                    print("Sorry, you should select from list only--(chicago new york city or washington) :")
                if input_type == 2:
                    print("Sorry, you should select from list only --( all or january, february, march, april, may, june )")
                if input_type == 3:
...
def get_filters():
    ...
    day = check_input("Please select day? (all or monday, tuesday, ... sunday)", 3)
    print('-'*40)
...
def load_data(city, month, day):
    ...
    if month != 'all':
        # use the index of the months list to get related int
...
def time_stats(df):
    ...
    print('Most Popular Common  Month:', popular_month)
...
def trip_duration_stats(df):
    ...
    # display maen travel time
    mean_travel_time = df['Trip Duration'].mean()

    print('Mean (average) Travel Time:', mean_travel_time)
...
def main():
    while True:
        ...
        records = input('\nWould you like to see the first 5 records of the trip type(yes) or to stop (no).\n')
